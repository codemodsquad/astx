...
export default class ASTTypesNodePath<T = Node> implements NodePath<T> {
  original: ASTPath<T>
  ...
  static wrap<T = Node>(original: ASTPath<T>): NodePath<T> {
    return new ASTTypesNodePath(original) as any
  }
  ...
  get parentPath(): NodePath | undefined {
    ...
    if (Array.isArray(parentPath.value)) {
      return ASTTypesNodePath.wrap(parentPath.parentPath)
    }
    return ASTTypesNodePath.wrap(parentPath)
  }
  ...
  get(key: string | number): NodePath | NodePath[] {
    ...
    if (Array.isArray(path.value)) {
      return path.filter(() => true).map(ASTTypesNodePath.wrap)
    } else {
      return ASTTypesNodePath.wrap(path) as any
    }
  }
